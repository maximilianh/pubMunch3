...
def findLinksWithUrlRe(page, searchRe):
    ...
    for linkUrl, linkText in page['links'].items():
        ...
        if searchRe.match(linkUrl):
            urls.append(linkUrl)
            logging.debug('Found link: %s -> %s' % (unidecode.unidecode(linkText), unidecode.unidecode(linkUrl)))
...
def getScopusIssns(publisherName):
    ...
    journalFname = io.TextIOWrapper(pkg_resources.resource_stream(__name__, 'data/journals.tab'))
...
def pageContains(page, strList):
    ...
    for text in strList:
        if text.encode('utf-8') in page["data"]:
...
def getMetaPdfUrl(page):
    ...
    if "citation_pdf_url" in htmlMetas:
        ...
        if not httpStartsWith("http://", pdfUrl):
            pdfUrl = urllib.parse.urljoin(page["url"], pdfUrl)
        return pdfUrl
    return None
...
class KargerCrawler(Crawler):
    ...
    def _httpGetKarger(self, url, delaySecs):
        ...
        if self.session is None:
            self.session = incapsula.IncapSession()
