...
class ErrorTests(ExRunTestCaseBase):
    def testCycleAll(self):
        ...
        try:
            ...
            self.assertEqual(_sortMsg(str(ex)), _sortMsg(expect))
        if ex == None:
    ...
    def testCycle(self):
        ...
        try:
            ...
            self.assertEqual(str(ex), "Production ErrorTests.ErrorTests.testCycle.file2 producedBy link already set")
        if ex == None:
    ...
    def testNoRule(self):
        ...
        try:
            ...
            self.assertEqual(str(ex), "No rule to build production(s): ErrorTests.ErrorTests.testNoRule.file3")
        if ex == None:
            self.fail("expected ExRunException")
